 ...
 * Author: Pavan T A
 *
 * Created on February 19, 2025, 9:49 AM
 */
...
void set_time(unsigned char key) {
    ...
    if (blink) {
        switch (blink_pos) {
            case 0:
                clcd_print("  ", LINE2(9)); // Clear seconds
                break;
            case 1:
                clcd_print("  ", LINE2(6)); // Clear minutes
                break;
        }
    } else {
        // Always update numbers when blink is OFF
...
char set_temp(unsigned char key) {
    ...
    if (key != ALL_RELEASED && key != '*' && key != '#') {
        ...
        if (temp > 180) {
            clcd_print("----------------", LINE1(0));
            clcd_print("    Invalid", LINE2(0));
            clcd_print("  Temperature", LINE3(0));
            clcd_print("----------------", LINE4(0));
            BUZZER = ON;
            __delay_ms(3000);
            BUZZER = OFF;
    ...
    if (blink) {
        clcd_print("   ", LINE2(8)); // Clear display when blinking
    }
    else {
